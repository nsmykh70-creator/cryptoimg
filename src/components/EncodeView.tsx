...
interface EncodeViewProps {
  ...
  // Lifted state — shared with DesktopRightPanel
  outputDataUrl: string | null;
  outputBlob: Blob | null;
  outputFormat: OutputFormat;
  selfTestSteps: SelfTestStep[];
  selfTestSummary: { pass: boolean; text: string } | null;
  onOutputChange: (dataUrl: string | null, blob: Blob | null, format: OutputFormat) => void;
  onSelfTestChange: (steps: SelfTestStep[], summary: { pass: boolean; text: string } | null) => void;
}

export const EncodeView: React.FC<EncodeViewProps> = ({ lang, argon2Params, outputDataUrl, outputBlob, outputFormat, selfTestSteps, selfTestSummary, onOutputChange, onSelfTestChange }) => {
  ...
  const handleImageFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        onOutputChange(null, null, outputFormat);
        onSelfTestChange(selfTestSteps.map(s => ({...s, status: 'idle' as const})), null);
      };
      img.src = dataUrl;
    };
  ...
  const updateStepStatus = (id: number, status: 'idle' | 'running' | 'success' | 'failed') => {
    onSelfTestChange(
      selfTestSteps.map((s) => (s.id === id ? { ...s, status } : s)),
      selfTestSummary
    );
  };
  ...
  const handleEncode = async () => {
    ...
    onSelfTestChange(selfTestSteps.map((s) => ({ ...s, status: 'idle' })), null);
    ...
    try {
      ...
      if (!matched) {
      onSelfTestChange(
        selfTestSteps.map((s) => (s.id === 5 ? { ...s, status: 'failed' } : s)),
        { pass: false, text: t.st_fail }
      );
        setIsProcessing(false);
        return;
      }
      onSelfTestChange(
        selfTestSteps.map((s) => (s.id === 5 ? { ...s, status: 'success' } : s)),
        { pass: true, text: t.st_pass }
      );
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          onOutputChange(url, blob, outputFormat);
          setProgressPct(100);
    ...
    <div className="h-full flex flex-col overflow-hidden bg-slate-900/40 select-none">
      ...
      <div className="w-full flex flex-col h-full overflow-y-auto p-4 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            {(['png', 'webp', 'avif'] as OutputFormat[]).map((fmt) => (
              <button
                key={fmt}
                type="button"
                onClick={() => onOutputChange(outputDataUrl, outputBlob, fmt)}
        ...
        <div className="pt-2 space-y-2">
          ...
          {/* Self-Test Audit & Output Preview — moved to DesktopRightPanel */}
        </div>
      </div>
